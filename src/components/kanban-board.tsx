...
import { Plus, Clock, User, AlertCircle, CheckCircle2, ArrowRight } from 'lucide-react'
      ...
      {(provided, snapshot) => (
        ...
        >
          <div className="flex items-start justify-between mb-2">
            ...
            <span className={`px-2 py-1 rounded text-xs font-mono ${priorityColors[task.priority]}`}>
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-blue-400 font-medium">{task.assignedAgent}</span>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-slate-800/50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-red-400">
              {getTasksByStatus('todo').filter(t => t.priority === 'critical').length}
            </div>
